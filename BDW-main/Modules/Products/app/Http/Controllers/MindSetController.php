...
use Modules\Products\app\Models\{MindsetQuestions, MindsetQuestionOption, MindsetQuestionOptionAnswer};
...
class MindSetController extends Controller
{
    ...
    {
        $questions = MindsetQuestions::select('id','question','image','is_enable')->with('options')->get();
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            if ($request->has('option') && is_array($request->option)) {
                ...
                MindsetQuestionOptions::insert($arr);
    ...
    {
        $question = MindsetQuestions::with('options')->find($id);
    ...
    {
        $previous_questions = MindsetQuestionOptions::whereIn('option', $request->option)->where('question_id', $id)->get()->pluck('id');
        ...
        try {
            ...
            $question = MindsetQuestions::findOrFail($id);
    ...
    {
        $question = MindsetQuestions::where('id', $id)->update([
            'is_enable' => $status
        ]);
